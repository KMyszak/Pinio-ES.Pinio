...
Oprogramowania Tasmota i moduł **ESP8266** obsługują wiele rodzajów sensorów, co pozwala podłączyć np. temperaturę, wilgotność czy ruch. Jedno wyjście **GPIO** może być przypisane do jednego typu czujnika, dlatego dla wielu różnych czujników należy używać osobnych portów.
...
Aby podłączyć czujniki różnego typu, należy skorzystać z dodatkowych **portów rozszerzeń** oraz dostępnych GPIO:
...
2. Przejdź do `Configuration → Module` i wybierz dla `GPIO2` opcję **DHT11**:  
    ...
    !!! Warning ""
        ...
        Wybór `GPIO2`, ponieważ `GPIO15` musi być w stanie **LOW** przy starcie.   
        Podłączenie czujnika do `GPIO15` może uniemożliwić uruchomienie urządzenia.
