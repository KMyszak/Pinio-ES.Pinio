...
**Tasmota** i moduły **ESP8266** obsługują wiele rodzajów czujników, co pozwala podłączyć np. temperaturę, wilgotność czy ruch. Jedno wyjście **GPIO** może być przypisane do jednego typu czujnika, dlatego dla wielu różnych sensorów należy używać osobnych portów.
...
1. `GPIO15` → musi być **LOW** przy starcie, inaczej ESP-12F nie wystartuje
2. `GPIO2` → musi być **HIGH** przy starcie, inaczej ESP-12F nie wystartuje
...
- czujniki high-impedance

### Elementy niebezpieczne
- przekaźniki podłączone bezpośrednio
...
2. Przejdź do `Configuration → Module` i wybierz dla `GPIO2` opcję **DHT11**:  
    ...
    !!! Warning "Wybór `GPIO2`, ponieważ `GPIO15` wymaga stanu **LOW** przy starcie i urządzenie może nie wystartować"
...
4. Po poprawnym podłączeniu powinien pojawić się odczyt na stronie czytnika oraz nowe encje w **Home Assistant**:  
